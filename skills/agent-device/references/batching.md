...
Success includes: `total`, `executed`, `totalDurationMs`, `results[]` with `step`, `command`, `durationMs`, and optional `data`.

Failure includes: `details.step`, `details.command`, `details.executed`, `details.partialResults`.
...
- Split long workflows into phases: navigate, verify/extract, cleanup.
