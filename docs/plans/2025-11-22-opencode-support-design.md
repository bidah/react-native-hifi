...
Add full react-native-hifi support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port react-native-hifi to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/react-native-hifi-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/react-native-hifi.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/react-native-hifi/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/react-native-hifi-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "react-native-hifi:brainstorming")')
  }),
...
1. **Inject using-react-native-hifi content**
   - Full content of the using-react-native-hifi skill
...
```javascript
// .opencode/plugin/react-native-hifi.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const React Native HiFiPlugin = async ({ client, directory, $ }) => {
  const react-native-hifiDir = path.join(process.env.HOME, '.config/opencode/react-native-hifi');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingReact Native HiFi = await readSkill('using-react-native-hifi');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingReact Native HiFi}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── react-native-hifi-codex        # UPDATED: Use skills-core
│   ├── react-native-hifi-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── react-native-hifi.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/react-native-hifi-codex`
...
2. Update `.codex/react-native-hifi-codex` to use shared core
...
1. Create `.opencode/plugin/react-native-hifi.js`
