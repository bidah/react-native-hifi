# React Native Space Release Notes
...
- `GEMINI.md` @imports `using-react-native-space` skill and tool mapping table at session start
- Gemini CLI tool mapping reference (`skills/using-react-native-space/references/gemini-tools.md`) — translates Claude Code tool names (Read, Write, Edit, Bash, etc.) to Gemini CLI equivalents (read_file, write_file, replace, etc.)
...
- Specs (brainstorming output) now save to `docs/react-native-space/specs/YYYY-MM-DD-<topic>-design.md`
- Plans (writing-plans output) now save to `docs/react-native-space/plans/YYYY-MM-DD-<feature-name>.md`
...
Writing-plans no longer offers a choice between subagent-driven and executing-plans. On harnesses with subagent support (Claude Code, Codex), subagent-driven-development is required. Executing-plans is reserved for harnesses without subagent capability, and now tells the user that React Native Space works better on a subagent-capable platform.
...
- `lib/brainstorm-server/` — WebSocket server with browser helper library, session management scripts, and dark/light themed frame template ("React Native Space Brainstorming" with GitHub link)
...
- Design spec and implementation plan in `docs/react-native-space/`
...
Added explicit priority ordering to using-react-native-space:

1. User's explicit instructions (CLAUDE.md, AGENTS.md, direct requests) — highest priority
2. React Native Space skills — override default system behavior
...
Added `<SUBAGENT-STOP>` block to using-react-native-space. Subagents dispatched for specific tasks now skip the skill instead of activating the 1% rule and invoking full skill workflows.
...
This fix should dramatically improve react-native-space skills compliance and should reduce the chances of Claude entering its native plan mode unintentionally.
...
**Using-react-native-space workflow graph intercepts EnterPlanMode**
...
Changed `async: true` to `async: false` in hooks.json. When async, the hook could fail to complete before the model's first turn, meaning using-react-native-space instructions weren't in context for the first message.
...
The `react-native-space-codex` bootstrap CLI, Windows `.cmd` wrapper, and related bootstrap content file have been removed. Codex now uses native skill discovery via `~/.agents/skills/react-native-space/` symlink, so the old `use_skill`/`find_skills` CLI tools are no longer needed.
...
The synchronous SessionStart hook blocked the TUI from entering raw mode on Windows, freezing all keyboard input. Running the hook async prevents the freeze while still injecting react-native-space context.
...
- Windows doesn't respect shebangs, so directly invoking the extensionless `react-native-space-codex` script triggered an "Open with" dialog. All invocations now prefixed with `node`.
...
React Native Space for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/react-native-space/` (see updated installation docs).
...
**Strengthened using-react-native-space skill for explicit skill requests**
...
The underlying skills (`react-native-space:brainstorming`, `react-native-space:executing-plans`, `react-native-space:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-react-native-space` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-react-native-space`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `react-native-space:brainstorming`)
...
**Skill priority in using-react-native-space**
...
- **Rationalizations table** in using-react-native-space - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-react-native-space is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > react-native-space
...
- Optimized react-native-space bootstrap to eliminate redundant skill execution. The `using-react-native-space` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-react-native-space` manually despite already having the content from session start.
...
- Added unified `react-native-space-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `react-native-space:skill-name` for react-native-space skills, `skill-name` for personal
...
- Updated terminology: "React Native Space skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/react-native-space-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/react-native-space-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core react-native-space functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-react-native-space skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-react-native-space/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-react-native-space skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-react-native-space/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `react-native-space:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `react-native-space:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `react-native-space:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `react-native-space:code-reviewer`
...
- All internal skill references now use `react-native-space:` namespace prefix
- Updated format: `react-native-space:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/react-native-space:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# React Native Space v2.0.0 Release Notes

## Overview

React Native Space v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([obra/react-native-space-skills](https://github.com/react-native-vibe-code/react-native-space)). This transforms react-native-space from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [obra/react-native-space-skills](https://github.com/react-native-vibe-code/react-native-space).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/react-native-space/skills/`
...
1. Your old `~/.config/react-native-space/.git` will be backed up to `~/.config/react-native-space/.git.bak`
2. Old skills will be backed up to `~/.config/react-native-space/skills.bak`
3. Fresh clone of obra/react-native-space-skills will be created at `~/.config/react-native-space/skills/`

### Removed Features

- **Personal react-native-space overlay system** - Replaced with git branch workflow
- **setup-personal-react-native-space hook** - Replaced by initialize-skills.sh
...
- Clones obra/react-native-space-skills on first run
...
- `REACT_NATIVE_SPACE_SKILLS_ROOT` set to `~/.config/react-native-space/skills`
...
- Removed obsolete setup-personal-react-native-space call from session-start
...
- Prominent link to react-native-space-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in obra/react-native-space-skills
- `scripts/` directory - Now in obra/react-native-space-skills/skills/using-skills/
- `hooks/setup-personal-react-native-space.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/react-native-space/skills
- `commands/brainstorm.md` - Updated paths to REACT_NATIVE_SPACE_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to REACT_NATIVE_SPACE_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to REACT_NATIVE_SPACE_SKILLS_ROOT
...
- PR #2: Personal react-native-space overlay system (later replaced)
...
```bash
# In Claude Code
/plugin marketplace add obra/react-native-space-marketplace
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/react-native-space/skills ~/react-native-space-skills-backup
   ```
...
- Skills repository is now at https://github.com/react-native-vibe-code/react-native-space
...
**Full Changelog:** https://github.com/react-native-vibe-code/react-native-space/compare/dd013f6...main
**Skills Repository:** https://github.com/react-native-vibe-code/react-native-space
**Issues:** https://github.com/react-native-vibe-code/react-native-space/issues
